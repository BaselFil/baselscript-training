...
2. read the routed `reference/semantics/*.md` file(s)
...
3. Check the routed `.def` files before using unfamiliar syntax; load `baselscript-language.json` only when the manifest routes it or full machine-contract validation is required.
4. Prefer regression-confirmed behavior over examples from old documentation.
5. If two sources disagree, apply the source authority defined in `reference/AI_CONTEXT.md`.
...
Do not require or load the entire BaselScript reference by default.

For each task:

1. read `reference/manifest.json`;
2. load every file in `baseline_required`;
3. classify all matching `task_routes`;
4. apply relevant `route_expansion_rules`;
5. load the union of those files only.

Load `reference/language/baselscript-language.json` only for a route that explicitly requires it,
for exact machine-wide validation, or when the smaller routed contract cannot resolve an unfamiliar
construct.
...
Do not target or advertise an arbitrary source-code line count. Code length must follow from the
required functionality.


## Canonical current spellings for generated code

When more than one historical spelling is accepted, generate the current canonical form below.
Preserve a different accepted spelling when modifying an existing verified script unless the user
asks for modernization.

```text
FORM/DIALOG input variable binding:  id=#variable
structured file declaration:         file name=<file> record=(...)
static LIST file source:             tile=file name=<file>
static LIST activation:              call list=<name>
FORM activation:                     draw form=<name>
```

For a runtime-created LIST, `create tile=file id=<file>` is regression-confirmed and remains the
canonical dynamic form.

For `write_record`, assign the variables declared in `record=(...)` and then call:

```baselscript
write_record file=<file>
```

Add `dir=` / other documented file options only when needed. Do not generate `values=(...)` or
`fields=(...)` for `write_record`.
