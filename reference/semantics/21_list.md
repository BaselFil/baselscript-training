...
For a static file-backed LIST, use this canonical spelling in newly generated code:

```baselscript
tile=file name=example_person
```

The runtime/regression corpus also contains accepted `tile=file id=...` forms. Treat `id=` as a
compatibility spelling for existing scripts, not as the preferred spelling for new static LIST code.

This rule concerns only the LIST source tile. It does not replace the structured `file ... record=...`
declaration.
