...
For new FORM code, bind an input to a BaselScript variable with `id=#variable`.

Canonical:

```baselscript
tile=input id=#first_name x=50 y=135 w=700 h=65
```

`name=#variable` occurs in older UI examples and may be accepted by historical/runtime paths,
but it is not the canonical spelling for newly generated FORM input code.

AI generation rule:

- generate `tile=input id=#variable`;
- preserve `name=` only when editing an existing verified script that already uses it;
- do not convert a working historical script merely for style.
...
For generated landscape layouts, treat approximately `1200 px` as the normal usable working width
inside the nominal 1280 px virtual width, unless a more specific FORM definition overrides it.
This leaves practical outer margins instead of filling the full nominal width.
...
tile=input id=#first_name x=60 y=135 w=650 h=65
```

Avoid:

```baselscript
tile=input id=#first_name x=40 y=135 w=1200 h=65
```
