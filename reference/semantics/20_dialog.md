...
DIALOG login
    ...
    tile=input1 id=#user hint="Name eingeben"

    tile=text2 text="Passwort"
    tile=input2 id=#password hint="Passwort eingeben" control=password
...
- For newly generated DIALOG input code, use `id=#variable` as the canonical variable binding.
...
- Generate `tile=inputN id=#variable` for new DIALOG code.
- Preserve `name=` only when modifying an existing verified historical script that already uses it.
